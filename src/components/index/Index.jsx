...
class IndexRoute extends React.Component {
    ...
    generateMenuRouter(menus, path) {
        ...
        path = path || '';
        menus.forEach((item) => {
            var p = item.key.length ? (path + ('/' + item.key)) : path;
            var _path = item.component ? this.getPathKey(p) : p;
            if (item.children && item.children.length) {
                routes.push(<Route key={item.key} breadcrumbName={item.title} path={_path}
                                   component={item.component}>{this.generateMenuRouter(item.children, p)}</Route>)
            } else {
                routes.push(<Route key={item.key} breadcrumbName={item.title} path={_path}
